...
- Every channel message wakes every idle non-human channel member except its author, regardless of who authored it. Members outside the channel never wake. A wake does not imply a reply, but **who wrote the message decides the default**, and the two directions pull opposite ways:
  - **A person's message defaults to an answer.** Someone writing in a channel is addressing the team and expects a reply even without an `@name`. This is the whole point of waking on un-mentioned messages; a silence rule that does not exempt people collapses the feature back to mention-only.
  - **A teammate's message defaults to silence.** Acknowledgements, thanks, completion reports and status updates must end the turn without posting. This is what stops agent-to-agent acknowledgement loops, which a live run actually produced.
- Both defaults live in two places that must agree: `TEAM_COLLABORATION_PROMPT` in `member-home.ts` and `formatMentionPrompt` in `member-lifecycle.ts`. The notification wins in practice because it is the last thing read before the member acts — an unconditional “then reply” there sustained an agent loop despite correct standing etiquette, and an unscoped silence rule there suppresses answers to people. It is the only place that knows the author's kind, so the human/teammate split is enforced there and merely restated in the standing prompt.
- Channel membership is opt-in for new channels and newly assigned members. The schema migration backfills pre-existing channels with their assigned members so the upgrade preserves their audience.
- Mentions resolve only among project members in the message's channel. Naming a project member outside that channel fails with a readable membership error.
- Ambient messages never interrupt a running member. Its channel cursor remains behind, and one consolidated wake after the run completes catches it up only on unread channels it belongs to, regardless of author. An explicit in-channel mention deliberately retains the existing interrupt behavior.
